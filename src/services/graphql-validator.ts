...
export class GraphQLQueryValidator {
  ...
  private static validateLinearAPIRules(info: GraphQLQueryInfo): ValidationResult {
    ...
    const definedParams = this.extractQueryParameters(info.queryString);
    ...
    definedParams.forEach((type, paramName) => {
      if (!this.isParameterUsedInQuery(info.queryString, paramName)) {
        warnings.push(
  ...
  public static validate(info: GraphQLQueryInfo): ValidationResult {
    const result = this.validateLinearAPIRules(info);
  ...
  public static validateStrict(info: GraphQLQueryInfo): void {
    const result = this.validate(info);
...
export function createValidatedGraphQLClient<T extends { rawRequest: Function }>(
  ...
  client.rawRequest = async function(
    query: string,
    variables?: Record<string, any>,
 ...
 */
export function applyLinearDefaults(
  variables: Record<string, any>
): Record<string, any> {
  return {
